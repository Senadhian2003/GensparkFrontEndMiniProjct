...
fetch(`http://localhost:5022/api/Sale/ViewAllRents`, {
    method: "GET"
}).then(res => res.json())
.then((data) => {
    allRents = data;
...
function searchFilter(rent) {
    ...
    return rent.user.name.toLowerCase().includes(searchTerm) || rent.rentId.toString().toLowerCase().includes(searchTerm);
}
